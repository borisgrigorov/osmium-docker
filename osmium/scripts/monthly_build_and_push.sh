...
do
  ...
  if docker run --pull=never --rm osmium:$VERSION 2>&1 | grep -q "osmium"; then
    ...
    docker tag osmium:$VERSION iboates/osmium:$VERSION
    docker push iboates/osmium:$VERSION
    echo -e "$VERSION: \033[32mPUSHED\033[0m"

    if [ "$LARGEST_VERSION" = "$VERSION" ]; then
      docker tag osmium:$VERSION iboates/osmium:latest
      docker push iboates/osmium:latest
